...
def entangling_layer(nqubits):
    ...
    #   CNOT  CNOT  CNOT...  CNOT
    for i in range(0, nqubits - 1, 2):  # Loop over even indices: epoch=0,2,...K-2
        qml.CNOT(wires=[i, i + 1])
    for i in range(1, nqubits - 1, 2):  # Loop over odd indices:  epoch=1,3,...K-3
        qml.CNOT(wires=[i, i + 1])
...
#
# # Count parameters for each part
# total_params = count_parameters(vqc)
