"""
Quantum Direct CFM v2 — Single-Gate SU(2^n) Encoding, No Brick-Layer
=====================================================================

Flow matching directly in pixel space WITHOUT a VAE, using a single
SU(2^n) exponential mapping gate on ALL n qubits. This eliminates the
brick-layer pattern, creating full entanglement across all qubits in
one gate — so every pairwise ANO observable sees rich correlations.

Key design: match d_flat ≈ enc_size for a ~1:1 enc_proj ratio.
  - 6 qubits, SU(64):  4095 generators, 2 Conv layers → d_flat=4096
  - 5 qubits, SU(32):  1023 generators, adjustable Conv layers
  - 4 qubits, SU(16):   255 generators, adjustable Conv layers

Differences from QuantumDirectCFM.py (v1):
  - Single SU(2^n) gate on all n qubits (no brick-layer even/odd groups)
  - Default 6 qubits with 2 Conv layers (d_flat≈4096 ≈ SU(64)=4095)
  - Default VQC = none (full entanglement from single gate)
  - Additive time conditioning with 256-dim sinusoidal embedding
  - enc_proj is nearly 1:1 by default

Architecture:
  Image x_t (3, H, W) + time t
    → ConvEncoder (2 stride-2 Conv layers) → flatten → d_flat (4096)
    ...
    → ANO pairwise measurement → C(n,2) observables
    → vel_head: Linear → SiLU → Linear → d_flat
    → ConvDecoder (2 stride-2 ConvTranspose layers) → velocity (3, H, W)

Usage:
  # Quantum SU(64) on 6 qubits (default, 1:1 enc_proj)
  python QuantumDirectCFM_v2.py --dataset=cifar10 --velocity-field=quantum

  # Quantum SU(32) on 5 qubits
  python QuantumDirectCFM_v2.py --n-qubits=5

  # With QViT ansatz (optional, adds entanglement on top)
  python QuantumDirectCFM_v2.py --vqc-type=qvit --vqc-depth=1

  # Classical baseline
  python QuantumDirectCFM_v2.py --velocity-field=classical
...
def get_args():
    p = argparse.ArgumentParser(
        description="Quantum Direct CFM v2 — Single-Gate SU(2^n) Encoding")
    ...
    # I/O
    p.add_argument("--job-id", type=str, default="qdcfm2_001")
...
def auto_enc_channels(img_size, n_qubits):
    ...
    Targets d_flat = 4^n (≈ enc_size = 4^n - 1) by choosing the number of
    stride-2 Conv layers and final channel count based on image resolution.
    ...
    Examples (n_qubits=6, target_d_flat=4096):
      32x32:   2 layers → [32, 64]         → 64×8×8  = 4096
      64x64:   3 layers → [32, 64, 64]     → 64×8×8  = 4096
      128x128: 5 layers → [32,64,128,256,256] → 256×4×4 = 4096
      256x256: 6 layers → [32,64,128,256,256,256] → 256×4×4 = 4096
    """
...
# ---------------------------------------------------------------------------
# 5. Quantum Circuit — Single SU(2^n) Gate Encoding
# ---------------------------------------------------------------------------
class SingleGateQuantumCircuit(nn.Module):
    """Quantum circuit with a single SU(2^n) encoding gate on all qubits.

    Unlike v1's brick-layer encoding, this uses ONE SU(2^n) exponential
    mapping gate that acts on ALL n qubits simultaneously, creating full
    entanglement across the entire register.

    For n=6: SU(64) with 4095 generators, 64x64 unitary matrix.
    """

    def __init__(self, input_dim, n_qubits, vqc_type, vqc_depth, k_local,
                 obs_scheme, qvit_circuit="butterfly", circuit_id=0):
        super().__init__()
        self.n_qubits = n_qubits
    ...
    def __init__(self, img_channels, img_size, enc_channels, n_circuits,
        ...
        for k in range(n_circuits):
            self.circuits.append(SingleGateQuantumCircuit(
                ...
                k_local=k_local, obs_scheme=obs_scheme,
...
def train_cfm(args, train_loader, val_loader, device):
    ...
    ckpt_path = os.path.join(ckpt_dir, f"ckpt_dcfm2_{args.job_id}.pt")
    csv_path = os.path.join(results_dir, f"log_dcfm2_{args.job_id}.csv")
    ...
    w_path = os.path.join(ckpt_dir, f"weights_dcfm2_{args.job_id}.pt")
...
def main():
    ...
    if args.velocity_field == "quantum":
        ...
        label = "Classical"
    print(f"=== Quantum Direct CFM v2 (Single-Gate SU Encoding) ===")
