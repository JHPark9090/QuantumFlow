"""
Quantum Latent Conditional Flow Matching v6 — Controlled Comparison
=====================================================================

A controlled experiment comparing Classical-A, single-circuit quantum (v6),
and multi-circuit quantum (v7) velocity fields with IDENTICAL classical parts.

Key design principles:
  - ALL velocity fields use the same time embedding (64-dim + time_mlp)
  - Same VAE (ResConvVAE, latent_dim=32) and data pipeline
  - Quantum circuits receive the FULL shared input (z_t + t_emb) — no splitting
  - enc_proj and vel_head use nonlinearities (SiLU), matching Classical-A style
  - Only difference: the velocity field core (MLP vs quantum circuit)

Velocity field variants:
  1. Classical-A (--velocity-field=classical):
     time_mlp(sin_embed(t,32)) -> concat(z_t[32], t_emb[32]) = 64
     -> Linear(64,256) -> SiLU -> Linear(256,256) -> SiLU
     -> Linear(256,256) -> SiLU -> Linear(256,32)

  2. v6 (--velocity-field=quantum --n-circuits=1):
     Same time embedding -> concat = 64
     -> enc_proj: Linear(64,256) -> SiLU -> Linear(256,105)
     -> SU(4) encoding (8q) -> QViT butterfly (depth=2) -> ANO pairwise k=2
     -> 28 obs -> vel_head: Linear(28,256) -> SiLU -> Linear(256,32)

  3. v7 (--velocity-field=quantum --n-circuits=8):
     Same time embedding -> concat = 64 (SHARED to all 8 circuits)
     -> Per circuit: enc_proj_k: Linear(64,256) -> SiLU -> Linear(256,105)
        -> SU(4) encoding (8q) -> QViT butterfly (depth=2) -> ANO pairwise k=2
        -> 28 obs
     -> concat 8*28 = 224 obs -> vel_head: Linear(224,256) -> SiLU -> Linear(256,32)
...
Usage:
  ...
  python QuantumLatentCFM_v6.py --phase=both --dataset=cifar10 \\
      --velocity-field=classical --epochs=200

  # v6: single quantum circuit (8q)
  python QuantumLatentCFM_v6.py --phase=both --dataset=cifar10 \\
      --velocity-field=quantum --n-circuits=1 --n-qubits=8 --epochs=200

  # v7: multi-circuit (8x8q, shared input)
  python QuantumLatentCFM_v6.py --phase=both --dataset=cifar10 \\
      --velocity-field=quantum --n-circuits=8 --n-qubits=8 --epochs=200
"""
...
def get_args():
    p = argparse.ArgumentParser(
        description="Quantum Latent CFM v6 — Controlled Comparison")
    ...
    p.add_argument("--latent-dim", type=int, default=32,
                   help="VAE latent dimension (default 32, matching Classical-A)")
    ...
    p.add_argument("--n-circuits", type=int, default=1,
                   help="Number of parallel quantum circuits (1=v6, 8=v7)")
    ...
    # Time embedding (same dim as latent for balanced input)
    ...
    # I/O
    p.add_argument("--job-id", type=str, default="qlcfm_v6_001")
...
# ---------------------------------------------------------------------------
# 4. Convolutional VAE (identical to Classical-A: ResConvVAE, latent_dim=32)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 5a. Classical Velocity Field (identical to Classical-A)
# ---------------------------------------------------------------------------
class ClassicalVelocityField(nn.Module):
    """Classical MLP velocity field — identical to Classical-A baseline.

    Architecture:
      sin_embed(t, 64) -> time_mlp(64->64->64) -> concat(z_t[32], t_emb[64]) = 96
      -> Linear(96,256) -> SiLU -> Linear(256,256) -> SiLU
      -> Linear(256,256) -> SiLU -> Linear(256,32)
    """
    ...
    def __init__(self, latent_dim=32, hidden_dims=(256, 256, 256),
        ...
        # Learned time embedding (same as Classical-A)
...
# ---------------------------------------------------------------------------
# 5b. Single Quantum Circuit Module (building block)
# ---------------------------------------------------------------------------
class SingleQuantumCircuit(nn.Module):
    """One quantum circuit with nonlinear enc_proj.

    Architecture per circuit:
      input(96) -> Linear(96,256) -> SiLU -> Linear(256,105) -> SU(4) encoding
      -> QViT/HWE VQC -> ANO measurement -> 28 obs
    """
    ...
    def __init__(self, input_dim, n_qubits, encoding_type, vqc_type,
        ...
        # Encoding projection WITH nonlinearity (matching Classical-A style)
    ...
    def forward(self, x):
        ...
        Args:
            x: (batch, input_dim) — full shared input (z_t + t_emb).
        Returns:
            q_out: (batch, n_obs) — measurement outcomes.
        """
        enc = self.enc_proj(x)  # Linear -> SiLU -> Linear
...
# ---------------------------------------------------------------------------
# 5c. Quantum Velocity Field (unified v6/v7)
# ---------------------------------------------------------------------------
class QuantumVelocityField(nn.Module):
    """Quantum velocity field with Classical-A's time embedding.

    When n_circuits=1: single circuit (v6).
    When n_circuits>1: multi-circuit with SHARED input (v7).

    Architecture:
      sin_embed(t, 64) -> time_mlp(64->64->64) -> concat(z_t[32], t_emb[64]) = 96
      -> [shared to all K circuits]
      -> Per circuit: enc_proj(96->256->105) -> SU(4) -> QViT -> ANO -> 28 obs
      -> concat K*28 obs -> vel_head(K*28->256->32)
    """
    ...
    def __init__(self, latent_dim, n_circuits, n_qubits, encoding_type,
        ...
        # Learned time embedding (identical to Classical-A)
        self.time_mlp = nn.Sequential(
            nn.Linear(time_embed_dim, time_embed_dim),
            nn.SiLU(),
            nn.Linear(time_embed_dim, time_embed_dim),
        ...
        # Input dimension: z_t + time_emb
        input_dim = latent_dim + time_embed_dim  # 32 + 64 = 96

        # Build K quantum circuits, each receiving the FULL shared input
        ...
        # Velocity head WITH nonlinearity (matching Classical-A style)
    ...
    def forward(self, z_t, t):
        ...
        """
        # Time embedding (identical to Classical-A)
        t_emb = self.time_mlp(self._time_embedding(t))
        z_combined = torch.cat([z_t, t_emb], dim=-1)  # (B, 96)

        # Run each circuit on the FULL shared input
        q_outputs = []
        for k in range(self.n_circuits):
            q_out_k = self.circuits[k](z_combined)  # (B, n_obs_per_circuit)
            q_outputs.append(q_out_k)

        # Concatenate all measurements
        q_all = torch.cat(q_outputs, dim=1)  # (B, total_obs)

        # Velocity head (with nonlinearity)
...
def train_vae(args, train_loader, val_loader, device):
    ...
    # Reconstruction quality metrics (computed on val set each epoch)
...
def train_cfm(args, vae, train_loader, val_loader, device):
    ...
    if args.velocity_field == "quantum":
        ...
        print(f"  enc_proj: Linear({input_dim},256) -> SiLU -> "
              f"Linear(256,{enc_per})")
